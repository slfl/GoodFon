...
#define IDM_NOTIFY_ON   120   /* включить уведомления */
#define IDM_SNOOZE_10   121
#define IDM_SNOOZE_30   122
#define IDM_SNOOZE_1H   123
...
#define IDC_CB_NOTIFYSTYLE 3030
...
typedef struct {
    ...
    int  max_attempts;
    int  notify;
    int  notify_style;                  /* 0 = системные (Windows), 1 = программные (своё окно) */
...
/* Инфо о текущей картинке с сайта (для карточки в трее) */
static int    g_cur_downloads = -1;       /* число скачиваний (-1 = неизвестно) */
static int    g_cur_rating = 0;           /* рейтинг картинки */
...
static unsigned g_fav_bag_sig = 0;
...
/* ================= ГСЧ (единый на процесс) =================
 * rand() в MSVCRT потоко-локальный: на каждый рабочий поток он пересевался, и
 * выбор из избранного брал лишь ПЕРВОЕ значение после посева — сильно
 * коррелированное с сидом. Из-за этого «случайность» залипала на нескольких
 * картинках. Используем один xorshift64*-генератор на весь процесс. */
static unsigned long long g_rng = 0;
static void rng_seed(void)
{
    LARGE_INTEGER li; QueryPerformanceCounter(&li);
    unsigned long long s = (unsigned long long)li.QuadPart * 0x9E3779B97F4A7C15ULL;
    s ^= (unsigned long long)time(NULL) << 16;
    s ^= (unsigned long long)GetCurrentProcessId() << 40;
    s ^= (unsigned long long)GetTickCount64();
    g_rng = s ? s : 0x106689D45497FDB5ULL;
}
static unsigned rng_next(void)
{
    ...
    g_rng = x;
    return (unsigned)((x * 0x2545F4914F6CDD1DULL) >> 33);
}
...
static void settings_write_defaults(void)
{
    ...
    reg_set_dword(L"notify_style", 0);
...
static int settings_load(void)
{
    ...
    g_cfg.notify_style = reg_get_dword(L"notify_style", 0);
...
static void ensure_toast_icons(void)
{
    ...
    g_ic_back   = make_toast_icon(L"\uE7A7", RGB(120, 110, 205));  /* Undo — индиго */
...
static WCHAR g_toast_text[300]  = L"";
static HICON g_toast_icon = NULL;

static void notify_core(const WCHAR *title, const WCHAR *text, HICON ic)
{
    if (!g_cfg.notify) return;
    if (g_notify_snooze_until && time(NULL) < g_notify_snooze_until) return;  /* отложены */

    if (g_cfg.notify_style == 1) {
        /* программное уведомление — рисуем своё окно в UI-потоке */
        wcsncpy(g_toast_title, title, 127); g_toast_title[127] = 0;
        wcsncpy(g_toast_text,  text,  299); g_toast_text[299]  = 0;
        g_toast_icon = ic;
        if (g_hwnd) PostMessageW(g_hwnd, WM_APP_TOAST, 0, 0);
        return;
    }
                                 ...
                                 WCHAR *out, int outsz)
{
    ...
    int pick = (idxs && m > 0) ? idxs[rng_below(m)] : rng_below(n);
...
/* Разобрать со страницы картинки: рейтинг, число скачиваний, путь голосования. */
static void parse_page_info(const char *html, int *rating, int *has_rating, int *downloads, char *vp, int vpsz)
{
    *rating = 0; *has_rating = 0; *downloads = -1; if (vp && vpsz) vp[0] = 0;
    /* рейтинг: ...class="vote"...>+24</span> — берём число ПОСЛЕ '>' закрытия тега
       (иначе можно поймать цифры из style="color:#005e00") */
    const char *v = strstr(html, "class=\"vote");
    if (!v) v = strstr(html, "class='vote");
    if (!v) v = strstr(html, "class=vote");        /* живой сервер отдаёт без кавычек */
    if (v) {
        const char *gt = strchr(v, '>');
        if (gt) {
            const char *q = gt + 1; const char *lim = q + 24;
            while (*q && q < lim && !((*q >= '0' && *q <= '9') || *q == '+' || *q == '-')) q++;
    ...
    /* скачивания: последнее число в user_block__user, склеивая разряды
       через разделитель тысяч (пробел / nbsp / &nbsp;) — «3 059» -> 3059 */
    const char *ub = strstr(html, "user_block__user");
    if (ub) {
        const char *end = strstr(ub, "</div>"); if (!end) end = ub + 400;
        int last = -1; const char *p = ub;
        while (p < end) {
            if (*p >= '0' && *p <= '9') {
                long val = 0;
                while (p < end) {
                    if (*p >= '0' && *p <= '9') { val = val * 10 + (*p - '0'); p++; }
                        ...
                        const char *q = p;
                        if (*q == ' ' || (unsigned char)*q == 0xA0) q++;
                        else if ((unsigned char)q[0] == 0xC2 && (unsigned char)q[1] == 0xA0) q += 2;
                        else if (!strncmp(q, "&nbsp;", 6)) q += 6;
                        else break;
                        if (q < end && *q >= '0' && *q <= '9') p = q; else break;
                    }
                }
                last = (int)val;
            } else p++;
    ...
    /* путь голосования: /api/fon/vote/{id}/ */
    const char *vpp = strstr(html, "/api/fon/vote/");
    if (vpp && vp && vpsz) {
        int i = 0; const char *p = vpp;
        while (*p && i < vpsz - 1) {
            vp[i++] = *p;
            if (*p == '/' && p > vpp && p[-1] >= '0' && p[-1] <= '9') break;
            p++;
        }
        vp[i] = 0;
    }
...
static void fallback_local(int favorite_only)
{
    ...
    if (!got) {
        ...
        notify_core(TW(L"GoodFon: ошибка", L"GoodFon: error"), TW(L"Нет доступных картинок.", L"No images available."), g_ic_warn);
        return;
    }
...
static unsigned fav_signature(FileEnt *f, int n)
{
    unsigned sig = (unsigned)n * 2654435761u;
    for (int i = 0; i < n; i++) {
        unsigned h = 2166136261u;
        for (const WCHAR *p = f[i].path; *p; p++) { h ^= (unsigned)*p; h *= 16777619u; }
        sig ^= h;   /* xor — порядок файлов не важен */
    }
    return sig;
}

/* Выбор из избранного «колодой»: каждая картинка показывается один раз за круг,
 * затем колода тасуется заново. Так участвуют все файлы, без залипаний. */
static int favorite_next(const WCHAR *cur, WCHAR *out, int outsz)
{
    int n = 0;
    FileEnt *files = dir_scan(g_favorite_dir, &n);
    if (!files || n == 0) { free(files); return 0; }

    unsigned sig = fav_signature(files, n);
    if (!g_fav_bag || n != g_fav_bag_n || sig != g_fav_bag_sig || g_fav_bag_cur >= g_fav_bag_n) {
        WCHAR (*nb)[MAX_PATH] = (WCHAR (*)[MAX_PATH])realloc(g_fav_bag, (size_t)n * sizeof(*nb));
        if (!nb) {   /* нет памяти — разовый честный выбор */
            int pick = rng_below(n);
            wcsncpy(out, files[pick].path, outsz - 1); out[outsz - 1] = 0;
            free(files); return 1;
        }
        g_fav_bag = nb;
        for (int i = 0; i < n; i++) { wcsncpy(g_fav_bag[i], files[i].path, MAX_PATH - 1); g_fav_bag[i][MAX_PATH - 1] = 0; }
        for (int i = n - 1; i > 0; i--) {           /* тасовка Фишера–Йейтса */
            int j = rng_below(i + 1);
            WCHAR t[MAX_PATH]; wcscpy(t, g_fav_bag[i]); wcscpy(g_fav_bag[i], g_fav_bag[j]); wcscpy(g_fav_bag[j], t);
        }
        if (n > 1 && cur && cur[0] && !_wcsicmp(g_fav_bag[0], cur)) { /* не начинать с текущей */
            WCHAR t[MAX_PATH]; wcscpy(t, g_fav_bag[0]); wcscpy(g_fav_bag[0], g_fav_bag[n - 1]); wcscpy(g_fav_bag[n - 1], t);
        }
        g_fav_bag_n = n; g_fav_bag_cur = 0; g_fav_bag_sig = sig;
    }
    free(files);

    WCHAR pick[MAX_PATH] = L"";
    while (g_fav_bag_cur < g_fav_bag_n) {           /* берём следующую существующую */
        const WCHAR *cand = g_fav_bag[g_fav_bag_cur++];
        if (GetFileAttributesW(cand) != INVALID_FILE_ATTRIBUTES) { wcscpy(pick, cand); break; }
    }
    if (!pick[0]) { g_fav_bag_sig = 0; return 0; } /* сбросить колоду на след. раз */
    wcsncpy(out, pick, outsz - 1); out[outsz - 1] = 0;
    return 1;
}
...
static int set_wallpaper_from_favorite(void)
{
    ...
    WCHAR cur[MAX_PATH]; get_current_wallpaper(cur, MAX_PATH);
    WCHAR chosen[MAX_PATH];
    if (!favorite_next(cur, chosen, MAX_PATH)) {
...
static void check_update(int silent, int install)
{
    ...
    if (h_self == h_new) {          /* байт-в-байт совпадает — обновлять нечего */
        ...
        if (!silent) notify_core(APP_NAME,
            TW(L"У вас последняя версия.", L"You have the latest version."), g_ic_ok);
        return;
    }
    ...
    upd_status(3);
    notify_core(APP_NAME, TW(L"Обновление установлено, перезапуск…",
                             L"Update installed, restarting…"), g_ic_newver);
...
static LRESULT CALLBACK SettingsProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_PAINT: {
        ...
        if (g_set_page == 1) {
            ...
            DrawTextW(dc, L"GoodFon 2.5", -1, &rt, DT_LEFT | DT_SINGLELINE);
            y += 34;
    ...
    case WM_COMMAND: {
        ...
        else if (id == IDC_CB_NOTIFYSTYLE && code == CBN_SELCHANGE) {
            int s = (int)SendMessageW(ctl, CB_GETCURSEL, 0, 0);
            g_cfg.notify_style = (int)SendMessageW(ctl, CB_GETITEMDATA, s, 0);
            reg_set_dword(L"notify_style", g_cfg.notify_style);
            /* показать пример выбранного стиля */
            notify_core(APP_NAME, g_cfg.notify_style == 1
                        ? TW(L"Программные уведомления включены", L"In-app notifications enabled")
                        : TW(L"Системные уведомления включены", L"System notifications enabled"),
                        g_ic_check);
        }
...
typedef struct {
    ...
    int arrow;              /* 1 = рисовать стрелку подменю справа */
} GfMenuItem;
static GfMenuItem g_mi[32];
static int   g_min = 0;
static HFONT g_menu_font = NULL, g_menu_icon = NULL, g_menu_sub = NULL, g_rate_icon = NULL;
...
static RECT  g_vote_up_rc = {0,0,0,0};  /* зоны стрелок рейтинга (коорд. окна меню) */
static RECT  g_vote_dn_rc = {0,0,0,0};
static HHOOK g_menu_hook = NULL;         /* мыш-хук на время открытого меню */
static volatile LONG g_vote_busy = 0;    /* идёт отправка голоса */
...
static void menu_add(HMENU m, const WCHAR *icon, const WCHAR *txt, UINT id, int checked, int disabled)
{
    ...
    it->checked = checked; it->disabled = disabled; it->sep = 0; it->card = 0; it->arrow = 0;
    AppendMenuW(m, MF_OWNERDRAW | (disabled ? MF_GRAYED : 0), id, (LPCWSTR)it);
    g_min++;
}
/* Пункт с подменю (флайаутом): рисуем стрелку справа. */
static void menu_add_popup(HMENU m, const WCHAR *icon, const WCHAR *txt, HMENU sub)
{
    GfMenuItem *it = &g_mi[g_min];
    it->text = txt; it->icon = icon; it->sub = NULL;
    it->checked = 0; it->disabled = 0; it->sep = 0; it->card = 0; it->arrow = 1;
    AppendMenuW(m, MF_OWNERDRAW | MF_POPUP, (UINT_PTR)sub, (LPCWSTR)it);
    g_min++;
}
...
static void menu_add_card(HMENU m, const WCHAR *name, const WCHAR *sub, UINT id, int disabled)
{
    ...
    it->checked = 0; it->disabled = disabled; it->sep = 0; it->card = 1; it->arrow = 0;
    g_card_it = it;
    AppendMenuW(m, MF_OWNERDRAW | (disabled ? MF_GRAYED : 0), id, (LPCWSTR)it);
    g_min++;
}
                                    ...
                                    LONG idObj, LONG idChild, DWORD idThread, DWORD tm)
{
    ...
    if (GetClassNameW(hwnd, cls, 16) && !wcscmp(cls, L"#32768")) {
        ...
        if (!g_menu_hwnd) g_menu_hwnd = hwnd;   /* только ГЛАВНОЕ меню — карточку в подменю не рисуем */
    }
}

/* ===== Этап 2: голосование за рейтинг (без закрытия меню) ===== */
static DWORD WINAPI vote_thread(LPVOID param)
{
    int dir = (int)(INT_PTR)param;
    char base[64]; base_url(base, sizeof(base));
    char url[256];  snprintf(url, sizeof(url), "%s%s", base, g_cur_vote_path);
    char body[16];  snprintf(body, sizeof(body), "m=%d", dir);
    char hdr[900];
    snprintf(hdr, sizeof(hdr),
        "Content-Type: application/x-www-form-urlencoded; charset=UTF-8\r\n"
        "X-Requested-With: XMLHttpRequest\r\n"
        "Origin: %s\r\n"
        "Referer: %s\r\n", base, g_cur_page_url[0] ? g_cur_page_url : base);
    HttpResp r;
    int ok = http_request("POST", url, body, hdr, 15000, 65536, &r) && r.status == 200;
    if (ok) LOG_INFO(T("Голос отправлен: m=%d, рейтинг=%d", "Vote sent: m=%d, rating=%d"), dir, g_cur_rating);
    else    LOG_WARN(T("Не удалось отправить голос (код %d).", "Failed to send vote (code %d)."), r.status);
    free(r.body);
    InterlockedExchange(&g_vote_busy, 0);
    return 0;
}

static void start_vote(int dir)
{
    if (!is_authorized() || !g_cur_vote_path[0]) return;   /* только после входа */
    if (g_cur_vote == dir) return;                          /* тот же голос — ничего */
    if (InterlockedCompareExchange(&g_vote_busy, 1, 0) != 0) return; /* уже идёт отправка */
    /* локальный пересчёт: 0→+1 (+1), 0→−1 (−1), флип +1↔−1 (±2) */
    g_cur_rating += (dir - g_cur_vote);
    g_cur_vote = dir;
    g_cur_has_rating = 1;
    if (g_hwnd) PostMessageW(g_hwnd, WM_APP_VOTED, 0, 0);   /* сразу перерисовать карточку */
    HANDLE t = CreateThread(NULL, 0, vote_thread, (LPVOID)(INT_PTR)dir, 0, NULL);
    if (t) CloseHandle(t); else InterlockedExchange(&g_vote_busy, 0);
}

/* Мыш-хук на время меню: клики по ▲/▼ обрабатываем и «поглощаем»,
   чтобы меню не закрывалось. */
static LRESULT CALLBACK MenuMouseProc(int code, WPARAM wp, LPARAM lp)
{
    if (code == HC_ACTION && g_menu_hwnd && (wp == WM_LBUTTONDOWN || wp == WM_LBUTTONUP)) {
        MOUSEHOOKSTRUCT *mh = (MOUSEHOOKSTRUCT *)lp;
        POINT p = mh->pt; ScreenToClient(g_menu_hwnd, &p);
        int overUp = PtInRect(&g_vote_up_rc, p);
        int overDn = PtInRect(&g_vote_dn_rc, p);
        if (overUp || overDn) {
            if (wp == WM_LBUTTONUP) start_vote(overUp ? 1 : -1);
            return 1;   /* поглощаем — меню остаётся открытым */
        }
    }
...
static void show_menu(void)
{
    if (!g_menu_font) {
        ...
        g_menu_icon = CreateFontW(-15, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET,
                                  0, 0, CLEARTYPE_QUALITY, 0, L"Segoe MDL2 Assets");
        g_rate_icon = CreateFontW(-12, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET,
                                  0, 0, CLEARTYPE_QUALITY, 0, L"Segoe MDL2 Assets");
    }
    ...
    g_marquee_off = 0; g_marquee_start = GetTickCount(); g_menu_hwnd = NULL;
    {
        ...
        g_marquee_tw = ns.cx;
        g_card_scroll = have_cur && (ns.cx > CARD_W - 80);   /* имя почти на всю ширину */
    }
    ...
    menu_add(m, g_paused ? L"\uE768" : L"\uE769",
             ...
             IDM_PAUSE, 0, 0);
    /* Уведомления: если сейчас отключены/отложены — «Включить», иначе «Отключить ▸» */
    {
        int snoozed = g_notify_snooze_until && time(NULL) < g_notify_snooze_until;
        if (!g_cfg.notify || snoozed) {
            menu_add(m, L"\uE7ED", TW(L"Включить уведомления", L"Enable notifications"), IDM_NOTIFY_ON, 0, 0);
        } else {
            HMENU sub = CreatePopupMenu();
            menu_add(sub, L"\uE823", TW(L"На 10 минут", L"For 10 minutes"), IDM_SNOOZE_10, 0, 0);
            menu_add(sub, L"\uE823", TW(L"На 30 минут", L"For 30 minutes"), IDM_SNOOZE_30, 0, 0);
            menu_add(sub, L"\uE823", TW(L"На 1 час",    L"For 1 hour"),    IDM_SNOOZE_1H, 0, 0);
            menu_add(sub, L"\uE823", TW(L"На 2 часа",   L"For 2 hours"),   IDM_SNOOZE_2H, 0, 0);
            menu_add(sub, L"\uE823", TW(L"На 4 часа",   L"For 4 hours"),   IDM_SNOOZE_4H, 0, 0);
            menu_add(sub, L"\uE711", TW(L"Насовсем",    L"Permanently"),   IDM_SNOOZE_OFF, 0, 0);
            menu_add_popup(m, L"\uE7ED", TW(L"Отключить уведомления", L"Disable notifications"), sub);
        }
    }
    ...
    g_menu_hook = SetWindowsHookExW(WH_MOUSE, MenuMouseProc, NULL, GetCurrentThreadId());
    TrackPopupMenu(m, TPM_RIGHTBUTTON, pt.x, pt.y, 0, g_hwnd, NULL);
    if (g_menu_hook) { UnhookWindowsHookEx(g_menu_hook); g_menu_hook = NULL; }
...
static void draw_card(HDC dst, RECT r, int sel)
{
    ...
    /* стрелка «открыть на сайте» — вверху справа */
    RECT ar = {c.right - 26, c.top + 5, c.right - 8, c.top + 23};
    DrawTextW(mdc, L"\uE8A7", -1, &ar, DT_RIGHT | DT_TOP | DT_SINGLELINE);
    SelectObject(mdc, oi);

    /* строка 1: имя (почти на всю ширину, бегущая строка) */
    int nRight = c.right - 28;
    HFONT ofn = (HFONT)SelectObject(mdc, g_menu_font);
    SetTextColor(mdc, nameC);
    RECT nr = {nx, c.top + 4, nRight, c.top + 23};
    if (g_card_scroll && !it->disabled) {
        HRGN clip = CreateRectRgn(nx, c.top + 3, nRight, c.top + 24);
        SelectClipRgn(mdc, clip);
        int x1 = nx - g_marquee_off;
        RECT t1 = {x1, c.top + 4, x1 + g_marquee_tw + 10, c.top + 23};
        DrawTextW(mdc, it->text, -1, &t1, DT_LEFT | DT_BOTTOM | DT_SINGLELINE | DT_NOCLIP);
        int x2 = x1 + g_marquee_tw + MARQUEE_GAP;
        RECT t2 = {x2, c.top + 4, x2 + g_marquee_tw + 10, c.top + 23};
    ...
    RECT sr = {nx, c.top + 23, c.right - 8, c.top + 38};
    DrawTextW(mdc, it->sub, -1, &sr, DT_LEFT | DT_TOP | DT_SINGLELINE | DT_END_ELLIPSIS);

    /* строка 3: слева «Скачиваний», справа блок рейтинга */
    if (g_cur_downloads >= 0 && !it->disabled) {
        WCHAR dl[48]; _snwprintf(dl, 48, TW(L"Скачиваний: %d", L"Downloads: %d"), g_cur_downloads);
        RECT dr = {nx, c.top + 38, c.right - 78, c.top + 53};
        DrawTextW(mdc, dl, -1, &dr, DT_LEFT | DT_TOP | DT_SINGLELINE | DT_END_ELLIPSIS);
    }
    SelectObject(mdc, ofn);

    if (g_cur_has_rating && !it->disabled) {
        COLORREF green = g_ui_theme ? RGB(90,200,120) : RGB(46,164,79);
        COLORREF red   = g_ui_theme ? RGB(232,120,140): RGB(214,74,106);
        COLORREF grey  = g_ui_theme ? RGB(110,110,110): RGB(180,180,180);
        /* стрелка тускнеет, если проголосовали в противоположную сторону */
        COLORREF upC  = (g_cur_vote == -1) ? grey : green;
        COLORREF dnC  = (g_cur_vote == 1)  ? grey : red;
        COLORREF numC = g_cur_rating > 0 ? green : g_cur_rating < 0 ? red
                       : (g_ui_theme ? RGB(150,165,180) : RGB(95,115,135));
        int rx1 = c.right - 8, rx0 = c.right - 72;
        int ry0 = c.top + 37, ry1 = c.bottom - 3;
        HFONT ori = (HFONT)SelectObject(mdc, g_rate_icon);
        SetTextColor(mdc, upC);
        RECT ru = {rx0, ry0, rx0 + 15, ry1};
        DrawTextW(mdc, L"\uE70E", -1, &ru, DT_LEFT | DT_VCENTER | DT_SINGLELINE);   /* ChevronUp */
        SetTextColor(mdc, dnC);
        RECT rd = {rx1 - 15, ry0, rx1, ry1};
        DrawTextW(mdc, L"\uE70D", -1, &rd, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);  /* ChevronDown */
        SelectObject(mdc, g_menu_sub);
        SetTextColor(mdc, numC);
        WCHAR rt[16]; _snwprintf(rt, 16, L"%+d", g_cur_rating);
        RECT rn = {rx0 + 14, ry0, rx1 - 14, ry1};
        DrawTextW(mdc, rt, -1, &rn, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        SelectObject(mdc, ori);
        /* зоны клика (координаты окна меню): расширяем по высоте для удобства */
        g_vote_up_rc = (RECT){ r.left + rx0 - 2, r.top + ry0, r.left + rx0 + 20, r.top + ry1 };
        g_vote_dn_rc = (RECT){ r.left + rx1 - 20, r.top + ry0, r.left + rx1 + 2, r.top + ry1 };
    } else {
        SetRectEmpty(&g_vote_up_rc); SetRectEmpty(&g_vote_dn_rc);
    }
...
/* ===== Программное уведомление: собственное окошко у трея =====
   Тема/язык — как во всём приложении; без звука; авто-скрытие с плавным
   затуханием; клик закрывает. Показывается только в UI-потоке. */
#define TOAST_W          348
#define TOAST_H          96
#define TOAST_DWELL_MS   4200
#define TOAST_TIMER_HIDE 0xD0
#define TOAST_TIMER_FADE 0xD1
static HWND  g_toast_wnd = NULL;
static HFONT g_toast_ft = NULL, g_toast_ft_t = NULL;
static HICON g_toast_appicon = NULL;
static int   g_toast_alpha = 255;

static LRESULT CALLBACK ToastProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
    switch (msg) {
    case WM_PAINT: {
        ...
        SetBkMode(mdc, TRANSPARENT);
        HBRUSH bg = CreateSolidBrush(cr_bg()); FillRect(mdc, &rc, bg); DeleteObject(bg);
        fill_round(mdc, rc, 12, cr_bg(), cr_border());   /* тело + рамка (углы скруглит DWM) */

        HICON ic = g_toast_icon ? g_toast_icon : (g_ic_info ? g_ic_info : g_toast_appicon);
        int tx = ic ? 62 : 20;
        if (ic) DrawIconEx(mdc, 16, (H - 32) / 2, ic, 32, 32, 0, NULL, DI_NORMAL);

        HGDIOBJ of = SelectObject(mdc, g_toast_ft_t);
        SetTextColor(mdc, cr_txt());
        RECT tr = { tx, 13, W - 14, 40 };
        DrawTextW(mdc, g_toast_title, -1, &tr, DT_LEFT | DT_SINGLELINE | DT_END_ELLIPSIS);
        SelectObject(mdc, g_toast_ft);
        SetTextColor(mdc, g_ui_theme ? RGB(180, 180, 180) : RGB(96, 96, 96));
        RECT br = { tx, 40, W - 14, H - 12 };
        DrawTextW(mdc, g_toast_text, -1, &br, DT_LEFT | DT_WORDBREAK | DT_END_ELLIPSIS);
        SelectObject(mdc, of);
    ...
    case WM_TIMER:
        if (wp == TOAST_TIMER_HIDE) {
            KillTimer(h, TOAST_TIMER_HIDE);
            g_toast_alpha = 255;
            SetTimer(h, TOAST_TIMER_FADE, 28, NULL);
        } else if (wp == TOAST_TIMER_FADE) {
            g_toast_alpha -= 22;
            if (g_toast_alpha <= 0) { KillTimer(h, TOAST_TIMER_FADE); ShowWindow(h, SW_HIDE); }
            else SetLayeredWindowAttributes(h, 0, (BYTE)g_toast_alpha, LWA_ALPHA);
        }
        return 0;
    case WM_LBUTTONUP:
        KillTimer(h, TOAST_TIMER_HIDE); KillTimer(h, TOAST_TIMER_FADE);
...
{
    if (!g_toast_ft) {
        g_toast_ft   = CreateFontW(-12, 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET,
                                   0, 0, CLEARTYPE_QUALITY, 0, L"Segoe UI");
        g_toast_ft_t = CreateFontW(-15, 0, 0, 0, FW_SEMIBOLD, 0, 0, 0, DEFAULT_CHARSET,
                                   0, 0, CLEARTYPE_QUALITY, 0, L"Segoe UI");
    }
    if (!g_toast_appicon)
        g_toast_appicon = (HICON)LoadImageW(g_hinst, MAKEINTRESOURCEW(IDI_APPICON),
                                            IMAGE_ICON, 32, 32, 0);
    if (!g_toast_wnd) {
        g_toast_wnd = CreateWindowExW(
            WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_NOACTIVATE | WS_EX_LAYERED,
            L"GoodFonToast", L"", WS_POPUP, 0, 0, TOAST_W, TOAST_H,
            NULL, NULL, g_hinst, NULL);
        if (!g_toast_wnd) return;
        int pref = DWMWCP_ROUND;
        DwmSetWindowAttribute(g_toast_wnd, DWMWA_WINDOW_CORNER_PREFERENCE, &pref, sizeof(pref));
    }
    RECT wa; SystemParametersInfoW(SPI_GETWORKAREA, 0, &wa, 0);
    int x = wa.right - TOAST_W - 12, y = wa.bottom - TOAST_H - 12;
    KillTimer(g_toast_wnd, TOAST_TIMER_FADE);
    KillTimer(g_toast_wnd, TOAST_TIMER_HIDE);
    g_toast_alpha = 255;
    SetLayeredWindowAttributes(g_toast_wnd, 0, 255, LWA_ALPHA);
    SetWindowPos(g_toast_wnd, HWND_TOPMOST, x, y, TOAST_W, TOAST_H, SWP_NOACTIVATE | SWP_SHOWWINDOW);
    InvalidateRect(g_toast_wnd, NULL, TRUE);
    ShowWindow(g_toast_wnd, SW_SHOWNOACTIVATE);
    SetTimer(g_toast_wnd, TOAST_TIMER_HIDE, TOAST_DWELL_MS, NULL);
}
...
static LRESULT CALLBACK WndProc(HWND h, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_DRAWITEM: {
        ...
        if (d->CtlType == ODT_MENU) {
            ...
            if (it->arrow) {   /* стрелка подменю справа */
                HFONT oi = (HFONT)SelectObject(d->hDC, g_menu_icon);
                RECT ar = r; ar.right -= 12;
    ...
    case WM_APP_VOTED:
        if (g_menu_hwnd) {   /* перерисовать карточку с новым рейтингом, не закрывая меню */
            ...
            draw_card(dc, g_card_rect, g_card_sel);
    ...
    case WM_COMMAND: {
        ...
        else if (id == IDM_NOTIFY_ON) {
            g_notify_snooze_until = 0;
            if (!g_cfg.notify) { g_cfg.notify = 1; reg_set_dword(L"notify", 1); }
            notify_core(APP_NAME, TW(L"Уведомления включены.", L"Notifications enabled."), g_ic_ok);
        }
        else if (id == IDM_SNOOZE_10 || id == IDM_SNOOZE_30 || id == IDM_SNOOZE_1H ||
                 id == IDM_SNOOZE_2H  || id == IDM_SNOOZE_4H) {
            int mins = id == IDM_SNOOZE_10 ? 10 : id == IDM_SNOOZE_30 ? 30 :
                       id == IDM_SNOOZE_1H ? 60 : id == IDM_SNOOZE_2H ? 120 : 240;
            /* уведомим ДО начала отсрочки, чтобы подтверждение показалось */
            WCHAR info[128];
            _snwprintf(info, 128, TW(L"Отложены на %d мин.", L"Snoozed for %d min."), mins);
            notify_core(APP_NAME, info, g_ic_check);
            g_notify_snooze_until = time(NULL) + (time_t)mins * 60;
        }
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE hPrev, PWSTR cmdline, int show)
{
    ...
    g_hinst = hInst;
    rng_seed();   /* единый ГСЧ на процесс, засев один раз */
    ...
    /* класс окна программных уведомлений */
    WNDCLASSW tc = {0};
    tc.lpfnWndProc = ToastProc;
    tc.hInstance = hInst;
    tc.lpszClassName = L"GoodFonToast";
